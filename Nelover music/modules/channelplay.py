...
async def m_cb(b, cb):
    ...
    if type_ == "cpause":
        ...
        if (chet_id not in callsmusic.active_chats) or (
            ...
            callsmusic.pause(chet_id)
            await cb.answer("تم الايقاف المؤقت")
    elif type_ == "ccls":
        await cb.answer("تم الاغلاق")
...
async def play(_, message: Message):
    ...
    if audio:
        ...
        keyboard = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton("قائمه الاغاني", callback_data="cplaylist"),
                    InlineKeyboardButton("القائمه", callback_data="cmenu"),
                ],
                [
                    InlineKeyboardButton(text="يوتيوب", url=f"{url}"),
                    InlineKeyboardButton(text="تنزيل", url=f"{dlurl}"),
                ],
    ...
    if chat_id in callsmusic.active_chats:
        ...
        await message.reply_photo(
            photo="final.png",
            reply_markup=keyboard,
            caption=" <b>جاري تشغيل</b> الاغنيه المطلوبه بواسطه {} عبر يوتيوب in Linked Channel".format(
...
async def jiosaavn(client: Client, message_: Message):
    global que
    lel = await message_.reply(" **جاري المعالجه**")
    try:
    ...
    query = text[1]
    res = lel
    await res.edit(f"جاري البحث عن `{query}` on jio saavn")
    try:
        ...
        [
            ...
            [
                InlineKeyboardButton(
                    text="انضم لقناه تحديثات البوت", url=f"https://t.me/{updateschannel}"
                )
            ],
            [InlineKeyboardButton(text="اغلاق", callback_data="ccls")],
        ]
    ...
    if chat_id in callsmusic.active_chats:
        ...
        await res.edit_text(f"{bn}=.....جاري التشغيل")
        que[chat_id] = []
        qeue = que.get(chat_id)
        s_name = sname
        r_by = message_.from_user
